...
int hooke(int nvars, double startpt[MAXVARS], double endpt[MAXVARS], double rho, double epsilon, int itermax)
{
    ...
    */
    ...
    keep = 1;
    #pragma omp parallel default(shared) reduction(+:funevals)
    {
      ...
      memcpy(&newxx,&newx,sizeof(newx));
			...
			for (int k = 0; k < nvars; k++) {
				...
				if (newx[k] <= xbefore[k])
					delta[k] = 0.0 - fabs(delta[k]);
				else
					delta[k] = fabs(delta[k]);
				/* now, move further in this direction */
				double ttmp = xbefore[k];
				xxbefore[k] = newxx[k];
				newxx[k] = newxx[k] + newxx[k] - ttmp;
			}
			ffbefore = newff;
        ...
        break;
			...
			for (int k = 0; k < nvars; k++) {
				mkeep = 1;
				if (fabs(newxx[k] - xxbefore[k]) > (0.5 * fabs(delta[k])))
					break;
				else
					mkeep = 0;
			}
...
int main(int argc, char *argv[])
{
  ...
  int size,pid,ypoloipo;
  MPI_Comm_rank(MPI_COMM_WORLD,&pid);
  MPI_Comm_size(MPI_COMM_WORLD,&size);
  ypoloipo = ntrials%size;//vriskw ypoloipo
  int pollaplasio = (int)(ntrials/size);

	t0 = get_wtime();
	for (trial = pid*pollaplasio; trial < ntrials; trial++) {
    if(trial > (pid+1) * pollaplasio && pid<size-1){
      break;
    }
    if((pid+1)*pollaplasio+ypoloipo > trial && pid == size-1){
      break;
    }
  ...
  if(pid == 0){
    ...
    for(int itr = 1; itr<size; itr++){
      MPI_Recv(&fx_pinakas[itr],1,MPI_FLOAT,itr,timh+itr,MPI_COMM_WORLD,&status);
    }
    //dialogh toy kaluterou fx
    double tmp_fx = fx_pinakas[0];//to prwto fx einai ths diergasias 0
    for(int itr = 1; itr<size; itr++){
      if(tmp_fx>fx_pinakas[itr]){
        tmp_fx = fx_pinakas[itr];
        temp_pid = itr;//shmeiwnw poia diergasia evgale to kalutero apotelesma
      }
    }//end for
    //enhmerwsh poia diergasia exei to kalutero apotelesma
    for(int itr = 1; itr<size; itr++){
      MPI_Send(&temp_pid,1,MPI_INT,itr,timh+itr,MPI_COMM_WORLD);
    }

  }else{
    //oi diergasies ektos ths 0 stelnoun best_fx
    MPI_Status status;
    MPI_Send(&fx_pinakas[pid],1,MPI_FLOAT,0,timh+pid,MPI_COMM_WORLD);
